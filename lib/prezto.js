...
prezto.prototype.run = function() {
  ...
  $(window).bind('resize',  function (ev) { self.runHooks('enter'); });
  this.slides.hide();
...
prezto.prototype.runHooks = function (type) {
  ...
  $.each(names, function(index, hook) {
    if (self.hooks[type][hook] != undefined) {
      return self.hooks[type][hook].call(self, self.currentSlide());
    }
  });
};
...
prezto.prototype.toString = function() {
  return [
    '(', this.current+1, '/', this.last+1, ') ',
    Math.round((this.current+1) * 100.0 / (this.last+1)), '%'
  ].join('');
